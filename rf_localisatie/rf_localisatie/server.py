...
def _parse_cells(iw_text):
    ...
    for b in blocks[1:]:
        ...
        m_sig = re.search(r"signal:\s*(-?\d+(?:\.\d+)?)\s*dBm", b)
...
def get_local_rssi():
    ...
    print(f"[LOCAL RSSI] {time.strftime('%H:%M:%S')}  RSSI={rssi} dBm")
    _last_scan_ts, _last_rssi = now, rssi
    return rssi
...
def data():
    ...
    # Bereken telefoonpositie als er 3 Pi's zijn
    px, py = compute_phone_position()

    # Voeg schatting van afstand toe als we maar 1 Pi hebben
    central_rssi = latest_data.get(1, {}).get("rssi", None)
    phone_distance = None
    if central_rssi is not None:
        phone_distance = rssi_to_distance(central_rssi)
...
def scan_once():
    try:
        iw_text = _iw_scan()
        cells = _parse_cells(iw_text)
        return jsonify([{"bssid": c["bssid"], "ssid": c["ssid"], "rssi": c["rssi"]} for c in cells])
    except Exception as e:

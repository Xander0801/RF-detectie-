...
# =============================
ip_to_key, unused_keys = {}, ANC_ORDER.copy()                   # dynamische IP→A/B/C mapping
...
# =============================
# UDP-listener (ontvang Pi-telemetrie)
# =============================
def listener():
    ...
    while True:
        ...
        # Eerste 3 unieke IP's krijgen sleutel A/B/C
        ...
        if key is None:
            continue  # (extra IP's negeren)

        # Voeg RSSI toe aan buffer + timestamp bijhouden
        try:
...
# =============================
# GUI (layout + render-loop)
# =============================
def main():
    # Start netwerklistener in achtergrondthread
    threading.Thread(target=listener, daemon=True).start()

    # Basis figure/axes. add_axes => posities in [left,bottom,width,height] fracs van fig.
    # Docs: https://matplotlib.org/stable/api/_as_gen/matplotlib.figure.Figure.add_axes.html
    ...
    # -- Rechtsboven: RAW-log (nieuwste boven) --------------------------------
    ax_raw = fig.add_axes([0.70, 0.64, 0.27, 0.30]); ax_raw.axis("off")
    raw_text = ax_raw.text(0.0, 1.0, "(no data)", va="top", family="monospace")

    # -- Links onder IP→Key: sliders voor rssi@1m en n per anker --------------
    # Slider API: https://matplotlib.org/stable/api/widgets_api.html
    sliders = []
    x_sl, w_sl = 0.02, 0.14
    ...
    for k in ANC_ORDER:
        for (label, vmin, vmax, v0, fld) in (
            ...
            # Let-op: capture k/fld als default-args zodat lambda juiste referenties bewaart
            sl.on_changed(lambda _v, kk=k, f=fld, s=sl: cal[kk].__setitem__(f, float(s.val)))
            sliders.append(sl); i += 1

    # -- Onder: TextBoxen om ankerposities aan te passen ----------------------
    ...
    # =============================
    # Render-loop (update kaart + tekst)
    # =============================
    while True:
        # Verval oude buffers
        now = time.time()
        ...
        # Update cirkels, trilateratie, overlay-informatie
        pts, dists, lines = [], [], []
        for k in ANC_ORDER:
            if rssi_buf[k]:
                med = float(np.median(np.asarray(rssi_buf[k], float)))  # robuuste ruisfilter (mediaan)
                ...
                lines.append(f"{k}: d={d:.2f}m • RSSI~{med:.1f} (1m={cal[k]['rssi1m']:.1f}, n={cal[k]['n']:.2f})")
        ...
        if len(pts) >= 3:
            try:
                ...
                est_dot.set_data([], []); info_txt.set_text(f"Trilateratie fout: {e}")
        else:
            est_dot.set_data([], []); info_txt.set_text((" | ".join(lines) if lines else "Wachten…") + "\n(≥3 ankers nodig)")

        # Bijwerken side-panels
        raw_text.set_text("\n".join(raw_log) if raw_log else "(no data)")
        if ip_to_key:
            lines_map = ["IP → Key:"]
            for ip, k in ip_to_key.items(): lines_map.append(f"  {ip:<15} → {k}")
            map_text.set_text("\n".join(lines_map))
        else:
